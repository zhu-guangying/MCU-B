...
//	IIC_Stop();
	...
	HAL_I2C_Master_Transmit(&hi2c1, OLED_ADDR , data, sizeof(data) / sizeof(data[0]), 100);
}
...
void OLED_ShowPicture(unsigned char p[])
{
  unsigned char x,y;
  unsigned int i=0;
  for(y=0;y<8;y++)
...
//x,y :起点坐标(x:0~127 y:0~63)
********************************************/
void OLED_ShowChinese(u8 x, u8 y, char (*s)[16], u8 n)
{      			    
	u8 t, i, adder = 0, flag = 0;
